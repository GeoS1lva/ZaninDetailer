...
from app.core.cache import brands_cache
...
class BrandService:
    ...
    async def list_brands(self, offset: int, limit: int) -> tuple[list[Brand], int]:
        ...
        cached = await brands_cache.get(key)
        if cached is not None:
            return cached
        result = await self._repo.list_all(offset=offset, limit=limit)
        await brands_cache.set(key, result)
        return result
    ...
    async def create_brand(self, data: BrandCreate, image: UploadFile | None) -> Brand:
        ...
        brand = await self._repo.create(data, image_url=image_url)
        await brands_cache.invalidate("list:")
        return brand
    ...
    async def update_brand(
        ...
        if image:
            if brand.image_url:
                await delete_image(brand.image_url)
            image_url = await upload_image(image)

        brand = await self._repo.update(brand, data, image_url=image_url)
        await brands_cache.invalidate("list:")
        return brand
